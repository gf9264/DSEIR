...
for edge in network1.G.edges:
    network1.G.add_edge(edge[0], edge[1], weight=random.uniform(0, 1))  # 可不可以作为权值 病毒的感染能力
for node in network1.G:
    network1.G.add_node(node, state=0)  # 用state标识状态 state=0 易感 ，state=1 潜伏 ， state=2 感染 ， state=3 免疫

seed = 53  # 选定Arkansas作为传染源
network1.G.nodes[seed]['state'] = 2  # 表示Arkansas是感染的
seed1 = 33
network1.G.nodes[seed1]['state'] = 2

seed2 = 47
network1.G.nodes[seed2]['state'] = 1
...
for i in range(max_iter_num):
    ...
    for v in all_infect_nodes:
        for nbr in network1.G.neighbors(v):  # v的邻居
            if network1.G.nodes[nbr]['state'] == 2:  # 如果这个邻居节点被感染
                edge_data = network1.G.get_edge_data(v, nbr)  # 得到边的权值
                if gamma < edge_data['weight']:  # 治愈概率
                    network1.G.nodes[nbr]['state'] = 3
                    new_remove.append(nbr)
                    if nbr not in all_remove_nodes:
                        new_remove.append(nbr)
            elif network1.G.nodes[nbr]['state'] == 1:  # 潜伏状态的节点
                edge_data = network1.G.get_edge_data(v, nbr)  # 得到边的权值
                if ita < edge_data['weight']:  # 传染概率
                    network1.G.nodes[nbr]['state'] = 2
                    new_infect.append(nbr)
                elif deta > edge_data['weight']:
                    network1.G.nodes[nbr]['state'] = 3
                    new_remove.append(nbr)
            elif network1.G.nodes[nbr]['state'] == 0:  # 如果这个邻居节点没被感染且没有被治愈
                edge_data = network1.G.get_edge_data(v, nbr)
                if beta3 < edge_data['weight']:
                    network1.G.nodes[nbr]['state'] = 1
    ...
    for i in all_exposed_nodes:
        p = np.random.rand()
        if p < ita:
            network1.G.nodes[i]['state'] = 2
            new_infect.append(i)
        elif p < deta:
            network1.G.nodes[i]['state'] = 3
            new_remove.append(i)

    for j in all_infect_nodes:
        p = np.random.rand()
        if p < gamma:
            network1.G.nodes[j]['state'] = 3
            new_remove.append(j)
...
draw_picture(network1.nums, max_iter_num, exposed, infect, recover)
